...
router.post('/', async (req, res) => {
  ...
  const { rows } = await pool.query(
    'INSERT INTO habits (user_id, title, icon) VALUES ($1, $2, $3) RETURNING *',
    [req.user.id, title, icon]
  );
...
router.post('/toggle', async (req, res) => {
  ...
  const existing = await pool.query(
    'SELECT id FROM habit_log WHERE habit_id = $1 AND date = $2', [habit_id, date]
  );

  if (existing.rows.length > 0) {
    await pool.query('DELETE FROM habit_log WHERE habit_id = $1 AND date = $2', [habit_id, date]);
    res.json({ toggled: false });
  } else {
    await pool.query(
      'INSERT INTO habit_log (habit_id, user_id, date) VALUES ($1, $2, $3)',
      [habit_id, req.user.id, date]
    );
